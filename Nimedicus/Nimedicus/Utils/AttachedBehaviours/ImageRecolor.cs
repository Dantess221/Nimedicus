...
namespace Nimedicus.Utils.AttachedBehaviours
{
    ...
    public class ImageRecolor
    {
        public static readonly DependencyProperty ColorShiftProperty = DependencyProperty.RegisterAttached(
            "ColorShift", typeof(Color), typeof(ImageRecolor), new PropertyMetadata(default(Color), OnColorShiftChanged));
        ...
        private static Dictionary<int, ImageSource> InitialImageSources = new Dictionary<int, ImageSource>();
        private static void ChangeColor(Image img)
        {
            ...
            {
                InitialImageSources.Add(imgHash, img.Source);
            }
        ...
        public static void ChangeImageColor(Image img, Color color)
        {
            ...
            if (imgAsBitMapSource != null)
            {
                ...
                byte[] pixels = new byte[imgAsBitMapSource.PixelWidth * imgAsBitMapSource.PixelHeight * 4];
                ...
                bmp.WritePixels(new Int32Rect(0, 0, imgAsBitMapSource.PixelWidth, imgAsBitMapSource.PixelHeight), pixels,
                    imgAsBitMapSource.PixelWidth * 4, 0);
                img.Source = bmp;
            }
        }

        public static void ChangeSVGColor(Image img, Brush brush, bool isColorShiftFill = true, bool isColorShiftStroke = true, bool isColorShiftInvisible = true)
        {
            var imgAsDrawingGroup = img.Source as DrawingImage;
            if (imgAsDrawingGroup != null)
            {
                ChangeColorForDrawing(imgAsDrawingGroup.Drawing, brush, isColorShiftFill, isColorShiftStroke, isColorShiftInvisible);
            }
        }

        public static void ChangeSVGColor(Image img, Color color, bool isColorShiftFill = true, bool isColorShiftStroke = true, bool isColorShiftInvisible = true)
        {
            ChangeSVGColor(img, new SolidColorBrush(color), isColorShiftFill, isColorShiftStroke, isColorShiftInvisible);
        }

        private static void SetFillForDrawingGroup(DrawingGroup group, Brush brush, bool isColorShiftFill, bool isColorShiftStroke, bool isColorShiftInvisible)
        {
        ...
        private static void ChangeColorForDrawing(Drawing drawing, Brush brush, bool isColorShiftFill, bool isColorShiftStroke, bool isColorShiftInvisible)
        {
            if (drawing is DrawingGroup)
                SetFillForDrawingGroup(drawing as DrawingGroup, brush, isColorShiftFill, isColorShiftStroke, isColorShiftInvisible);
            ...
            else if (drawing is GeometryDrawing)
            {
                ...
                {
                    geo.Brush = brush; // For changing FILL
                }
