...
namespace Nimedicus.Utils
{
        ...
        INotifyPropertyChanged
    {
        ...
        {
            TValue t;
            return TryGetValue(key, out t) ? t : default(TValue);
        }
        public bool Any(Func<DictionaryEntry, bool> predicate)
        {
            foreach (var item in _keyedEntryCollection)
            {
                if (predicate(item))
                {
                    return true;
        ...
        public void ForEachValue(Action<TValue> action)
        {
            foreach (var item in _keyedEntryCollection)
            {
                action.Invoke((TValue)item.Value);
            }
        }
        public bool AnyValue(Func<TValue, bool> predicate)
        {
            foreach (var item in _keyedEntryCollection)
            {
                if (predicate((TValue)item.Value))
                {
                    return true;
                }
            }
        ...
        {
            var inpropItem = value as INotifyPropertyChanged;
            if (inpropItem != null)
            {
                inpropItem.PropertyChanged += FireCollectionItemPropertyChanged;
            }

            var inpropExtItem = value as INotifyPropertyChangedExtended;
            if (inpropExtItem != null)
            {
                inpropExtItem.PropertyChangedExtended += FireCollectionItemPropertyChangedExtended;
            }
        ...
        protected virtual bool RemoveEntry(TKey key)
        {
            ...
            var inpropItem = _keyedEntryCollection[key].Value as INotifyPropertyChanged;
            if (inpropItem != null)
            {
                inpropItem.PropertyChanged -= FireCollectionItemPropertyChanged;
            }

            var inpropExtItem = _keyedEntryCollection[key].Value as INotifyPropertyChangedExtended;
            if (inpropExtItem != null)
            {
                inpropExtItem.PropertyChangedExtended -= FireCollectionItemPropertyChangedExtended;
            }
        ...
        protected virtual bool SetEntry(TKey key, TValue value)
        {
            ...
            if (keyExists)
            {
                prevKeyIndex = _keyedEntryCollection.IndexOf(_keyedEntryCollection.FirstOrDefault(ke => ke.Key.Equals(key)));
            ...
            {
                _keyedEntryCollection.Insert(prevKeyIndex, new DictionaryEntry(key, value));
            }
        ...
        object IDictionary.this[object key]
        {
            get
            {
                if (key is TKey && _keyedEntryCollection.Contains((TKey)key))
                {
        ...
        #region IEnumerable
        public IEnumerator GetCrutchEnumerator()
        {
            return GetSimpleEnumerator();
        }
        ...
        protected KeyedDictionaryEntryCollection _keyedEntryCollection = new KeyedDictionaryEntryCollection();

        protected int _countCache = 0;
        protected Dictionary<TKey, TValue> _dictionaryCache = new Dictionary<TKey, TValue>();
        protected int _dictionaryCacheVersion = 0;
        ...
        public bool RemoveAll(Func<KeyValuePair<TKey, TValue>, bool> condition)
        {
            var keysToDelete = this.Where(condition).ToArray();
            var foundAndDeleted = false;

            foreach (KeyValuePair<TKey, TValue> kvp in keysToDelete)
            {
                foundAndDeleted = Remove(kvp.Key) || foundAndDeleted;
            }
        ...
        public bool MoveNext()
        {
            ...
            //    throw new Exception("Collection was changed and iteration cannot continue");

            for (; (uint)index < (uint)_keyedEntryCollection.Count; index = index + 1)
            {
                if (_keyedEntryCollection.GetHashCode() >= 0)
                {
                    current = new KeyValuePair<TKey, TValue>((TKey)_keyedEntryCollection[index].Key, (TValue)_keyedEntryCollection[index].Value);
                    index = index + 1;
                    return true;
                }
            }
            index = _keyedEntryCollection.Count + 1;
        ...
        public KeyValuePair<TKey, TValue> Current
        {
            get
            {
                if (index == 0 || index == _keyedEntryCollection.Count + 1)
                    throw new Exception("Enum index is out of bounds");
        ...
        public virtual void Dispose()
        {
            if (AutoDispose)
            {
                ...
                CollectionItemPropertyChanged = null;
                PropertyChanged = null;

                foreach (KeyValuePair<TKey, TValue> pair in this)
                {
                    if (typeof(IDisposable).IsAssignableFrom(typeof(TKey)))
                    {
        ...
        public virtual IEnumerator<KeyValuePair<TKey, TValue>> GetSimpleEnumerator()
        {
        ...
        private bool _firePropertyChanged = true;
        public virtual void SetEnablePropertyChanged(bool isEnabled, bool sendAllPropertyChanged = true)
        {
            _firePropertyChanged = isEnabled;
            if (isEnabled && sendAllPropertyChanged)
            {
                foreach (var value in Values)
                {
                    FireCollectionItemPropertyChanged(value, new PropertyChangedEventArgs(null));
                    FireCollectionItemPropertyChangedExtended(value, new PropertyChangedExtendedEventArgs(null, typeof(TValue), null, null));
                }
            }
        }
        public event PropertyChangedEventHandler CollectionItemPropertyChanged;

        public void FireCollectionItemPropertyChanged(object s, PropertyChangedEventArgs e)
        {
            if (_firePropertyChanged)
            {
                CollectionItemPropertyChanged?.Invoke(s, e);
            }
        }

        public event PropertyChangedExtendedEventHandler CollectionItemPropertyChangedExtended;

        public void FireCollectionItemPropertyChangedExtended(object s, PropertyChangedExtendedEventArgs e)
        {
            if (_firePropertyChanged)
            {
                CollectionItemPropertyChangedExtended?.Invoke(s, e);
            }
        }
    }
}
